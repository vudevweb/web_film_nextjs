...
function CardMovie({ movies , domain , totalPages , slug , page}) {
     const router = useRouter();
     const [currentPage, setCurrentPage] = React.useState(page);
     const handlePageChange = (page) => {
          setCurrentPage(page);
          // router.push(`/danh-muc/${slug}?page=${page}`);
          window.location.href = `/danh-muc/${slug}?page=${page}`;
     };
                              ...
                              <div key={index} className="col-6 col-sm-4 col-lg-3 col-xl-2">
                                   <div className="card text-center bg-none mt-1" style={{ padding: 10 }}>
                                        <Link href={`/phim/${movie.slug}`} className="card__cover">
                                             <img loading="lazy" src={domain + '/' + movie.poster_url} alt={movie.name} height={265} />
                                        ...
                                        <h3 className="card__title"><Link href={`/phim/${movie.slug}`}>{movie.name}</Link></h3>
                                   </div>
                              </div>
                         ))}
