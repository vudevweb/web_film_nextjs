...
function CardMovie({ movies , domain , totalPages , slug , page , baseUrl}) {
     const router = useRouter();
     const [currentPage, setCurrentPage] = React.useState(page);
     const handlePageChange = (page) => {
          setCurrentPage(page);
          // router.push(`/danh-muc/${slug}?page=${page}`);
               ...
               <div className="row">
                         {movies.map((movie, index) => (
                              <div key={index} className="col-6 col-sm-4 col-lg-3 col-xl-2">
                                   <div className="card text-center bg-none mt-1">
                                        <a href={`/phim/${movie.slug}`} className="card__cover">
                                             <img loading="lazy" src={domain + '/' + movie.poster_url} alt={movie.name} height={280} />
                                        ...
                                        <h3 className="card__title"><a href={`/phim/${movie.slug}`}>{movie.name}</a></h3>
                                   </div>
                              </div>
                         ))}
